...
def scale_OLI_ST(image):
        #opticalBands = image.select(['SR_B2', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B6', 'SR_B7']).multiply(0.0000275).add(-0.2).rename(['Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2'])
        thermalBands = image.select('ST_B.*').multiply(0.00341802).add(149.0).subtract(273.15)
        return image.addBands(thermalBands, None, True)#.addBands(opticalBands, None, True)

def scale_MODIS_ST(image):
    thermalBands = image.select('LST_.*').multiply(0.02).subtract(273.15)
    return image.addBands(thermalBands, None, True)#.addBands(thermalBands, None, True)