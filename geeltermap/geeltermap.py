...
class Map(ipyleaflet.Map):
    ...
    def __init__(self, **kwargs):
        ...
        print("Please choose a tool and then wait one minute after selecting the national network.")
        ...
        tools = {
            ...
            "list": {
                "name": "Form",
                "tooltip": "This will be the future Map form",
                "button_style":"danger",
            },            
            "leaf": {
                "name": "PhenoApp",
            ...
            "thermometer-empty": {
                "name": "LST",
                "tooltip": "Land Surface Temperature Tool",
    ...
    def add_elter_sites(self, site, style, name):
        ...
        data = deims.getSiteById(site)

        if len(data)== 0:
            data = deims.getListOfSites(site)
            for i in data:
                ndata = deims.getSiteById(i)
                name = ndata['title']
                deimsID = ndata['id']['suffix']
                try:
                    #area = deims.getSiteBoundaries(i)['geometry'][0].area
                    a = deims.getSiteBoundaries(deimsID)
                    b = gdf_to_ee(a)
                    self.add_ee_layer(b, style, name)
                except Exception as e:
                    #print(e)
                    continue
            ...
            name = data['title']
            deimsID = data['id']['suffix']
            a = deims.getSiteBoundaries(deimsID)
            b = gdf_to_ee(a)
            self.add_ee_layer(b, style, name)
        else:
            print('Error: any site was found with this ID')
